...
def video(file):
    ...
    videoo = cv2.VideoWriter(DR_PATH+f'/kyjj11_{name}_output_.mp4', fourcc, fps, (int(width_2), int(height_2)))
    ...
    while True:
        ...
        with open(DR_PATH+f'/kyjj11_{name}_meta_info.json','w',encoding='utf-8') as make_file:
            json.dump(meta_info, make_file, ensure_ascii=False, indent='\t')

        with open(DR_PATH+f'/kyjj11_{name}_key_info.json','w',encoding='utf-8') as make_file:
