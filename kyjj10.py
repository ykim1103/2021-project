...
def video(file):
    ...
    videoo = cv2.VideoWriter(f'/data/kyj_dt/kyjj10_{name}_output_.mp4', fourcc, fps, (int(width_2), int(height_2)))
    ...
    while True:
        ...
        for i in range(len(total_keys22)):
            ...
            for j in range(len(total_keys22[i])):
                key_info[str(i)][j] = {'class':text_test[int(total_class22[i][j])], 'key':total_keyss[i][j],'boxcorners':total_boxes22[i][j].tolist()}
                
                
        with open(f'kyjj10_{name}_meta_info.json','w',encoding='utf-8') as make_file:
            json.dump(meta_info, make_file, ensure_ascii=False, indent='\t')

        with open(f'kyjj10_{name}_key_info.json','w',encoding='utf-8') as make_file:
